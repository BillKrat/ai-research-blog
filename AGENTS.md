...
- **Prefer explicit file/symbol references over pronouns or "the above."** A smaller model's effective context window and instruction-following are both weaker; concrete anchors (`IPasswordHasher` in `src/AiBlogResearch.Security/`, not "the hasher we talked about") reduce the chance of it editing or reasoning about the wrong thing.
