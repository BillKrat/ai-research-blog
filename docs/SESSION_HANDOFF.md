...
**Machine-to-machine auth for WebApi → MCP Server calls (decided 2026-09-17, ahead of actually building item 2):** yes, doable, and it doesn't need anything from SmarterASP itself — Auth0 supports this directly via a **Machine-to-Machine Application** using OAuth2's **Client Credentials Grant** (register an M2M app tied to an "API"/audience per MCP server; WebApi/MCP Host requests a token with `client_id`/`client_secret`, caches it for its lifetime, attaches it as a Bearer header on each stateless MCP request; the MCP server validates it via Auth0's JWKS same as a user token, just with a service identity in `sub`). This is a separate Auth0 "Application" registration from the user-facing one WebApi already validates tokens from — not the same app wearing two hats. Low overhead because the token is cached (not re-fetched per call) and validation is local JWT signature checking (no per-request Auth0 network round trip). **When actually building `McpServer.Database`/`McpServer.FileSearch`: give each its own Auth0 API/audience (or distinct scopes on one API)**, so a token minted for one doesn't also work against the other — matches the already-decided "each MCP server holds its own credentials" boundary. Unverified: current Auth0 free-tier M2M token-request quota — check the dashboard when this milestone actually starts rather than assuming a number.
